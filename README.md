...
Новый интерфейс доступен отдельно и не заменяет стабильную страницу на `/`.
React-preview уже содержит обзор, позиции, динамику стоимости, доходность,
лидеров изменения, историю цен, календарь и пассивный доход, а также формы RUB,
вкладов, валюты и ручных бумаг. В разделе **Данные** доступны импорт T‑Invest,
обновление цен и курсов, снимки, Excel, дата начала учёта и приватные Git-бэкапы.

Docker собирает этот интерфейс автоматически. Для локальной сборки без Docker
потребуется Node.js:
...
Старый интерфейс остаётся на <http://127.0.0.1:8000/> как страховочный вариант,
пока React-preview проходит практическую обкатку на реальном портфеле.
...
Форма и CLI сначала используют имеющийся ручной RUB. Если его недостаточно,
только недостающая сумма записывается как внешнее пополнение, после чего вся
сумма атомарно переводится во вклад. Поэтому проценты, погашения и выручка от
продаж можно реинвестировать без повторного увеличения показателя «Вложено».
Для операции, которая не должна автоматически добавлять недостающий RUB,
используйте `POST /api/ledger/deposits/open` или MCP tool
`open_deposit_with_cash`: они требуют уже имеющийся ручной остаток.
...
Производный показатель себестоимости в уже сохранённых снимках можно безопасно
проверить и исправить по журналу операций. Первый запуск ничего не меняет;
`--apply` предварительно создаёт целостный SQLite-бэкап, а повторное применение
не меняет уже исправленные снимки:

```bash
python -m app.cli repair-cost-basis --from-date 2026-07-14
python -m app.cli repair-cost-basis --from-date 2026-07-14 --apply
```
...
| `FETCH_EVERY_MIN` | `0` | Интервал импорта свежих операций и обновления цен Т‑Инвестиций |
