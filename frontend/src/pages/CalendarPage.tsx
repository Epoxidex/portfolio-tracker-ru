...
function EventDetails({event}:{event:CalendarEvent}) { const style=TYPE_STYLE[event.type]||fallback; const Icon=style.icon; return <Stack gap="xl"><Paper className="event-detail-card" radius="xl" p="xl" style={{background:style.soft,"--event-accent":style.hex} as React.CSSProperties}><ThemeIcon color={style.color} variant="filled" size={48} radius="lg"><Icon size={23}/></ThemeIcon><Text c={style.color} size="sm" fw={750} mt="lg">{event.type}</Text><Title order={2} mt={4}>{event.instrument}</Title><Text className="drawer-amount" mt="xl">{formatMoney(event.amount)}</Text></Paper><Stack gap="md"><DetailRow label="Дата выплаты" value={formatDate(event.date)}/><DetailRow label="Инструмент" value={event.ticker||event.instrument}/><DetailRow label="Тип события" value={event.type}/></Stack><Text size="xs" c="dimmed" lh={1.6}>Будущие события являются прогнозом. Фактическое зачисление отражается отдельной операцией или после синхронизации с брокером.</Text></Stack>; }
